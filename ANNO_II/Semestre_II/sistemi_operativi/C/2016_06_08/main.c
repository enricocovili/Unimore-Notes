...
int main(int argc, char **argv)
{
  ...
  if ((Fcreato = open("/tmp/creato", O_CREAT | O_WRONLY | O_TRUNC, 0644)) < 0) /* usato la open in versione estesa per azzerare il file nel caso esista gia' */
  {
    printf("Errore nella creazione del file %s\n", "/tmp/creato");
    exit(3);
  }

  N = argc - 2; /* numero di file */

  srand(time(NULL)); /* impostazione seme randomico per generazione numeri casuali */
    ...
    if (pid == 0)
    {
      ...
      while (read(fd, &linea[j], 1))
      {
        if (linea[j] == '\n')
        {
          ...
          printf("DEBUG-figlio %d, riga lungh: %d\n", n, j);
          int deb = 0;
          if (deb = read(pipePf[n][0], &indice_linea_random, sizeof(indice_linea_random)) != sizeof(indice_linea_random))
          {
            printf("Errore comunicazione Figlio-Padre n.%d. deb=%d\n", n, deb);
            exit(-1);
          }
          printf("DEBUG-figlio n.%d. Ricevuto indice random:%d\n", n, indice_linea_random);
          if (indice_linea_random < j)
          {
            write(Fcreato, &linea[indice_linea_random], 1); /* scrivo quel carattere sul file */
            caratt_scritti++;
          }
          j = 0;
        }
  ...
  for (j = 0; j < H; j++)
  {
    ...
    {
      printf("sizeof(j).%d\n", sizeof(j));
      read(pipeFp[n][0], &lung_linea, sizeof(lung_linea));
