...
int main(int argc, char **argv)
{
    ...
    int i, j;           /* iteratori utilizzati nei cicli */
    int fd;             /* variabile usata dal figlio per open file */
    long int tot;       /* numero totale di caratteri letti */
    char buff[MSGSIZE]; /* array per contenere la stringa da convertire */
    int L;              /* indice per la scrittura della stringa */
    int lunghezza;      /* variabile per memorizzare il risultato dell'atoi */
    ...
    for (i = 0; i < N; i++)
    {
        ...
        { /* codice che viene eseguito dal figlio */
            ...
            { /* codice del nipote */
                ...
                perror("Problemi di esecuzione di wc da parte del nipote");
            ...
            L = 0; /* inizializzo il conteggio */
            while (read(p[0], &(buff[L]), sizeof(char)) > 0)
            { /*provo a leggere dal nipote il numero scritto come stringa*/
                if (buff[L] == '\n')
                    break;
                L++;
            }
            ...
            if (L != 0)
            {
                buff[L] = 0;            /* metto il terminatore di stringa */
                lunghezza = atoi(buff); /* converto la stringa numerica in numero int */
            }
            else
                lunghezza = 0;

            /* scrivo sull'unica pipe aperta il numero di linee */
            write(pipes[i][1], &lunghezza, sizeof(lunghezza));
    ...
    tot = 0; /* inizializzo il contatore */
    ...
    for (i = 0; i < N; i++)
    {
        if (read(pipes[i][0], &lunghezza, sizeof(lunghezza)) == sizeof(lunghezza))
            tot += lunghezza; /* aggiungo al totale la lunghezza */
    }

    printf("Il padre ha ricevuto il conteggio totale di %ld\n", tot);
