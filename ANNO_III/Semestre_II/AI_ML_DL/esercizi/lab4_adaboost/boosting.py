...
class WeakClassifier:
    ...
    def fit(self, X: np.ndarray, Y: np.ndarray):
        ...
        self._threshold = np.random.uniform(X[:, self._dim].min(), X[:, self._dim].max())
    ...
    def predict(self, X: np.ndarray):
        ...
        y_pred = np.where(X[:, self._dim] >= self._threshold, 
                           -self._label_above_split, 
                           self._label_above_split)
...
class AdaBoostClassifier:
    ...
    def fit(self, X: np.ndarray, Y: np.ndarray, verbose: bool = False):
        ...
        for l in range(self.n_learners):
            ...
            cur_idx = np.random.choice(a=n, size=int(0.5*n), p=sample_weights)
            ...
            while error > 0.5:
                ...
                error = np.mean(sample_weights[cur_idx] * (y_pred != cur_Y))
            ...
            labels_weakclass = cur_wclass.predict(X)
            sample_weights = np.mean(sample_weights * np.exp(-alpha * Y * labels_weakclass))
    ...
    def predict(self, X: np.ndarray):
        ...
        for i in range(num_samples):
            for j in range(self.n_learners):
                y_pred[i] += self.alphas[i] * self.learners[i].predict(X[i:i])
