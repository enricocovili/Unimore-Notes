...
#     file_id="1SagLh5XNSV4znhlnkLRkV7zHPSDbOAqv", dest_path="./data/got.zip", unzip=True
# )
...
def load_got_dataset(path, train_split=0.8):
    ...
    test_idx = np.random.choice(
        np.arange(0, total_characters),
        size=(n_test_characters,),
        replace=False,
        p=test_sampling_probs,
    )
    x_test = X[test_idx]
    y_test = Y[test_idx]
    ...
    train_idx = np.random.choice(
        np.arange(0, total_characters),
        size=(n_train_characters,),
        replace=True,
        p=train_sampling_probs,
    )
    x_train = X[train_idx]
    y_train = Y[train_idx]
